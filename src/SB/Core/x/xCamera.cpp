...
static void _xCameraUpdate(xCamera* cam, F32 dt)
{
    ...
    while (xcam_do_collis && sCamCollis)
    {
        ...
        xRayHitsGrid(&colls_oso_grid, globals.sceneCur, &ray, SweptSphereHitsCameraEnt, &sws.qcd,
                     &sws);
...
// NONMATCH("https://decomp.me/scratch/c9iST")
{
    ...
    if (dCA > s2_radius)
    {
        ...
        F32 fov = DEG2RAD(1.0f) * xCameraGetFOV(camera) * 0.5f + cfg.margin_angle;
...
template <>
SHARED_INLINE F32 range_limit<F32>(F32 v, F32 minv, F32 maxv)
{
    if (v <= minv)
    {
        return minv;
