...
// The displacement vector, already in texture-coordinate units and already
// scaled by the effect's strength, in xy. c1 rather than c0 because librw
// reserves c0 for the fog colour.
#define PSTAIL float4 displace : packoffset(c1);
...
Texture2D swirl : register(t1);
SamplerState swirlSampler : register(s1);
...
float4 main(VS_out input) : SV_TARGET
{
	...
	float d = swirl.Sample(swirlSampler, uv).r * 2.0f - 1.0f;

	float4 c = screen.Sample(screenSampler, uv + d * displace.xy);
	DoAlphaTest(c.a);
	return c;
}
