...
//
// One function, and it is not implemented. This file says why, in the place a
// reader will look for it, and is where the implementation goes.
//
...
// and box triangle tests -- so the maths already exists and is tested.
//
...
// each RpWorldSector it reaches. Here rootSector is always NULL, because the
// only thing that would build one is RpWorldStreamRead, which is not written
// either -- see world.cpp. librw has no world sector code at all: no
// RpWorldSector counterpart, no plane sectors, no world chunk reader.
//
// So the two are blocked on one missing piece and have to be written together:
// a world reader that produces sectors (world.cpp says what that involves,
// including that BFBB's own BSPs are GameCube-native), and RpWorldSector's
// tree. That is why the struct in rpworld.h keeps RenderWare's own layout
// rather than being mirrored: there is no librw type to mirror it onto, and
// inventing the plane-sector node here would put a guess in a public header
// that a real reader would have to match.
//
// Until then the player and everything else walks through the level geometry
// and falls through the floor. Model-to-model collision still works -- that is
// RpAtomicForAllIntersections, which xClumpColl and the JSP path go through.
