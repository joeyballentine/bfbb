// The binding tables, the presets and the parser: everything about a binding
// that needs no config file and no device. bfbb_config links this and the
// game does too; iPadBind.cpp is the half that reads config.ini and asks the
// pad in hand. The grammar and the argument for it are in iPadBind.h.
...
const char* iPadBindPresetEntry(const char* preset, S32 row)
{
    ...
    {
        if (iHostStrCaseCmp(kPadBindPresets[i].name, preset) == 0)
        {
            const char* bound = kPadBindPresets[i].pad[row];
            return bound != NULL ? bound : kPadBindButtons[row].pad;
        }
    }

    return kPadBindButtons[row].pad;
}
