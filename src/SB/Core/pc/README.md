...
Each is off unless set, and each costs a load per frame when it is.
    ...
    BFBB_FONTDUMP=<f>   write each font's atlas, character set and glyph boxes
                        to a file, appending one record per font. It is what
                        `tools/fontfit` reads: everything the substitution knows
                        about the font it is replacing, so the same sweep the
                        game runs at startup can be run outside it and printed
                        as a table. Works with no font configured, which is the
                        point -- the dump comes first.
...
| `iFX` (1 fn) | **done** | `iFXanimUVCreatePipe` returns the texture-coordinate-transform pipeline added to the librw fork, with its own render in front of librw's to read the four `xFXanimUV*` globals into the matrix at draw time — the same shape as `RxGameCubeAllInOneSetRenderCallBack` on the console. Both backends apply it in a vertex shader variant; a backend with no pipeline answers NULL, which xFX.cpp:883 handles by leaving the surface unanimated |
...
| `ngcrad3d` | not ported, and cannot apply | RAD Game Tools' 3D surface layer, not radiosity: it is the GameCube blitter that puts a decoded Bink frame on screen, and it includes `bink/include/rad3d.h` and `iFMV.h` to do it. The port decodes the Xbox `.xmv` through FFmpeg, so no Bink frame is ever produced for it to draw |
...
- **`iSnapshot.h`** and **`rw/snapshot.cpp`** keep the last presented frame as
  a texture, so that `zGameTakeSnapShot` -- an empty function in the shipped
