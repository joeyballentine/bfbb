...
U32 iCSFileOpen(xCutscene* csn)
{
    ...
    {
        // PORT: retail turns a DVD sector delta into a byte offset --
        // (ainfo.sector - startAddr) << 5, where startAddr is where the HIP
        // begins on the disc. There is no disc here and tag_iFile has no
        // DVDFileInfo, so this is the same arithmetic without it.
        //
        // It comes out identical rather than approximate. xpkrsvc derives both
        // fields from one byte offset -- sector = base_sector + d_off / 32 and
        // plus_offset = d_off % 32 -- and the host iFileGetInfo reports a base
        // sector of 0 because seeking is free, so sector * 32 + plus_offset is
        // exactly d_off again. Retail drops plus_offset, which is safe only
        // while assets happen to be 32-aligned; they are not always, so it is
        // kept here.
        iFileSeek(&csn->File, headerskip + (ainfo.sector << 5) + ainfo.plus_offset,
                  IFILE_SEEK_SET);
