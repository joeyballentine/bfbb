...
void zNPCTiki_ExplodeFX(zNPCTiki* tiki)
{
    ...
    if (tiki->SelfType() == NPC_TYPE_TIKI_THUNDER)
    {
        ...
        for (i = 0; i < zsc->num_npcs; i++)
        {
            if (zsc->npcs[i] == tiki || !((zNPCCommon*)zsc->npcs[i])->flg_vuln)
                continue;

            xVec3Sub(&delta, (xVec3*)&tiki->model->Mat->pos,
                     (xVec3*)&(zsc->npcs[i])->model->Mat->pos);
            F32 dist2 = xVec3Length2(&delta);
            if (dist2 < 25.0f)
            {
                if ((((zNPCCommon*)zsc->npcs[i])->SelfType() & ~0xFF) == 'NTT\0')
                    ((zNPCTiki*)zsc->npcs[i])->timeToLive = dist2 * 0.012f;
                ((zNPCCommon*)zsc->npcs[i])->Damage(DMGTYP_THUNDER_TIKI_EXPLOSION, 0, 0);
            }
        }

        for (i = 0; i < zsc->num_dyns; i++)
        {
            if ((zsc->dyns[i])->baseType != 0x1B)
                continue;

            xVec3Sub(&delta, (xVec3*)&tiki->model->Mat->pos,
                     (xVec3*)&((xEnt*)zsc->dyns[i])->model->Mat->pos);
            if (xVec3Length2(&delta) < 25.0f)
                zEntEvent(zsc->dyns[i], 0x3A);
        }
...
static void zNPCTiki_PickTikisToAnimate()
{
    ...
    xNPCBasic* npc; // dwarf says there's 2 `xNPCBasic* npc`s but that causes a register issue
    for (; coll < cend; coll++)
    {
...
void zNPCTiki::Init(xEntAsset* entass)
{
    ...
    if (this->model->Anim != NULL)
    {
        this->tikiAnim = *this->model->Anim->Single->State->Data->RawData;
    ...
    case zNPCCARRY_ATTEMPTPICKUP:
        if (this->numChildren != 0)
        {
            ...
            for (S32 i = 0; i < 4; i++)
            {
                ...
                {
                    if (this->children[i]->bound.box.box.lower.y <
...
void zNPCTiki::Process(xScene* xscn, F32 dt)
{
    ...
    {
        F32 duration = iAnimDuration(this->tikiAnim);
    ...
    xVec3 pos_save;
    xVec3Copy(&pos_save, (xVec3*)&this->model->Mat->pos);
    ...
    if (this->numParents)
    {
        for (i = 0; i < 4; i++)
        {
            if (i != this->contactParent && this->parents[i])
            {
                ...
                else if (this->parents[i]->bound.box.box.upper.y >
                         this->parents[p]->bound.box.box.upper.y)
                {
                    this->contactParent = i;
                }
            }
    ...
    if (this->tikiFlag & 1)
    {
        ...
        this->model->Mat->pos.y += dh;
        this->bound.box.box.lower.y += dh;
        this->bound.box.box.upper.y += dh;
        this->bound.box.center.y += dh;
        ...
        else
        {
            if (this->bound.box.box.lower.y < this->landHt)
            {
                ...
                if (this->tikiFlag & 2)
                {
                    this->Damage(DMGTYP_DAMAGE_SURFACE, 0, 0);
                }
    ...
    if (this->numParents != 0)
    {
        ...
        if (!(this->tikiFlag & 1))
        {
            ...
            F32 dy = parentBound.box.box.upper.y - bound.box.box.lower.y;
    ...
    {
        xVec3 playerDist;
        xVec3Sub(&playerDist, (xVec3*)&globals.player.ent.model->Mat->pos,
                 (xVec3*)&(this->model->Mat->pos));
        playerDist.y = 0.0f;
        F32 offset = xVec3Length2(&playerDist);
    ...
    if (scale)
    {
        ...
        xVec3SMul(&scaledUpper, &this->origLocalBound.box.box.upper, this->model->Scale.x);
        xVec3SMul(&scaledLower, &this->origLocalBound.box.box.lower, this->model->Scale.x);
        xVec3SMul(&scaledCenter, &this->origLocalBound.box.center, this->model->Scale.x);

        xVec3Add(&this->bound.box.box.upper, (xVec3*)&this->model->Mat->pos, &scaledUpper);
    ...
    else
    {
        xVec3Add(&this->bound.box.box.upper, (xVec3*)&this->model->Mat->pos,
                 &this->origLocalBound.box.box.upper);
        xVec3Add(&this->bound.box.box.lower, (xVec3*)&this->model->Mat->pos,
                 &this->origLocalBound.box.box.lower);
        xVec3Add(&this->bound.box.center, (xVec3*)&this->model->Mat->pos,
                 &this->origLocalBound.box.center);
    }

    xVec3Sub(&this->deltaPos, (xVec3*)&this->model->Mat->pos, &pos_save);
    ...
    if (this->numChildren != 0)
    {
        for (i = 0; i < 4; ++i)
        {
            if (this->children[i])
            {
                this->children[i]->ParentUpdated(this);
            }
        }
    }
...
void zNPCTiki::FindParents(zScene* zsc)
{
    ...
    if ((c.flags & 1) != 0)
    {
        if (c.optr != NULL)
        {
            ...
            xVec3 temp;
            xVec3Sub(&temp, &ray.origin, (xVec3*)&((xEnt*)c.optr)->model->Mat->pos);
            temp.y -= c.dist;
            ...
            this->nonTikiParentDisp.x =
                xVec3Dot(&temp, (xVec3*)&this->nonTikiParent->model->Mat->right);
            this->nonTikiParentDisp.y =
                xVec3Dot(&temp, (xVec3*)&this->nonTikiParent->model->Mat->up);
            this->nonTikiParentDisp.z =
                xVec3Dot(&temp, (xVec3*)&this->nonTikiParent->model->Mat->at);
    ...
    F32 oldLower, oldUpper;
    if ((this->tikiFlag & 0x20) != 0)
    {
        oldLower = this->bound.box.box.lower.y;
        oldUpper = this->bound.box.box.upper.y;

        this->bound.box.box.lower.y = oldLower / 2;
        this->bound.box.box.lower.y += 0.5f * this->bound.box.box.upper.y;
        this->bound.box.box.upper.y = (this->bound.box.box.lower.y + oldUpper) - oldLower;
    }
    ...
    for (S32 i = 0; i < zsc->num_npcs; i++)
    {
        ...
        zNPCTiki* tiki = (zNPCTiki*)npc;
        if ((tiki->tikiFlag & 0x300) != 0)
            continue;
        if ((tiki->tikiFlag & 0x10) != 0)
            continue;
        if (tiki->bound.box.box.upper.y < this->landHt)
            continue;

        if (!(this->bound.box.box.upper.x > tiki->bound.box.box.lower.x + 0.1f))
            continue;
        if (!(tiki->bound.box.box.upper.x > this->bound.box.box.lower.x + 0.1f))
            continue;
        if (!(this->bound.box.box.upper.z > tiki->bound.box.box.lower.z + 0.1f))
            continue;
        if (!(tiki->bound.box.box.upper.z > this->bound.box.box.lower.z + 0.1f))
            continue;

        if (!(0.75f * tiki->bound.box.box.upper.y + 0.25f * tiki->bound.box.box.lower.y <
              this->bound.box.box.lower.y + 0.00001f))
            continue;
        if (!(tiki->bound.box.box.upper.y <
...
static S32 thunderIdleCB(xGoal* rawgoal, void*, en_trantype* trantype, F32 dt, void*)
{
    ...
    factor = tiki->t1;
    factor = (factor * factor + 1.0f) - factor;
    tiki->model->RedMultiplier = factor;
    tiki->model->BlueMultiplier = factor;
...
static void loveyTikiRender(xEnt* ent)
{
    ...
    if (model == NULL || !xEntIsVisible(ent) || model->Flags & 0x400)
        return;
    ...
    if (iModelCullPlusShadow(model->Data, model->Mat, &center, &shadowResult) == 0)
    {
        F32 dot = 2.0f;
        if ((globals.player.ControlOff & 0x23f3) == 0)
        {
    ...
    if ((factor < 0.5f || cache.polyCount == 0) && shadowResult == 0)
