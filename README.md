...
| Smoothed geometry | `[experimental] hipoly_assets` | Tessellates the level and its models into curved PN-triangle patches as they load, so rocks, coral, trees and characters lose their facets while floors and walls stay where they were. The collision tree is rebuilt to match. Nothing on disk changes; loads take a few seconds longer. `src/SB/Core/pc/iHipolyTess.cpp` says what it does to a mesh. |
