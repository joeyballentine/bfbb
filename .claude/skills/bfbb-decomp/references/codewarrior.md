...
- **The stack layout rule, stated precisely: CW orders locals by *size descending*, ties broken by declaration order, at *descending* addresses.** This supersedes the older "reverse the declaration list" heuristic, which is just the special case where every local is the same size. Confirmed on three functions in one session; it predicted `CollidePyramidBoxTop`'s frame exactly. Reversing the declaration list still fixed Sandy's 3852-byte `Process`, but only because its locals were uniform. If your `r1+offset`s are permuted, sort by size first.
